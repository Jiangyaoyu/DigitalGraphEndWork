...
def evaluate_one_image(image_array):
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            if max_index == 0:
                result = ('这是玫瑰花的可能性为： %.6f' % prediction[:, 0],'玫瑰')
            elif max_index == 1:
                result = ('这是郁金香的可能性为： %.6f' % prediction[:, 1],'郁金香')
            elif max_index == 2:
                result = ('这是蒲公英的可能性为： %.6f' % prediction[:, 2],'蒲公英')
            else:
                result = ('这是这是向日葵的可能性为： %.6f' % prediction[:, 3],'向日葵')
            return result
...
if __name__ == '__main__':
    ...
    img = Image.open('./input_data/sunflowers/6953297_8576bf4ea3.jpg')
    # plt.imshow(img)
    # plt.show()
